...
var url = "https://cdn.contentful.com/spaces/"+process.env.CONTENTFUL_SPACE_ID+"/environments/"+"master"+"/entries?access_token="+process.env.CONTENTFUL_ACCESS_TOKEN;
...
request(projectConfigurationsUrl, function (error, response, body) {
  if (!error && response.statusCode == 200) {
    var data = JSON.stringify(JSON.parse(body));  // curTemp holds the value we want
    writeFile(path.join('contentful/response-projectConfigurations.json'),data);
  }
});